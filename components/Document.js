...
export default function Document() {
  ...
  return (
    <section id="documents" className="scroll-mt-24 bg-white p-4 md:p-6 relative">
      <h2 className="text-xl font-bold text-slate-800 mb-6 font-sans">Documents</h2>
      
      {/* FIXED: Changed grid-cols-1 to grid-cols-2 so it is a grid layout on mobile */}
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6">
        {documents.map((doc) => (
          ...
          >
            ...
            <div 
              className="p-2 md:p-3 pb-3 md:pb-4 bg-white flex flex-col cursor-pointer"
              onClick={(e) => handleOpenAction(e, doc)}
            >
              
              {/* PURE CSS EMBEDDED GRAPHIC */}
              <div className="relative w-full aspect-[16/10] bg-gradient-to-br from-slate-50 to-slate-100 border border-slate-200/60 rounded-t-lg overflow-hidden flex flex-col items-center justify-center gap-1 md:gap-2 select-none">
                ...
                <svg 
                  className={`w-8 h-8 md:w-10 md:h-10 text-slate-300 transition-transform duration-300 group-hover:scale-110 ${doc.type === 'image' ? 'group-hover:text-emerald-500' : 'group-hover:text-rose-400'}`} 
                  viewBox="0 0 24 24" 
                  fill="none" 
                  stroke="currentColor" 
                  strokeWidth="1.5"
                ...
                <span className="text-[8px] md:text-[10px] font-bold tracking-widest text-slate-400 bg-white px-1.5 md:bg-white md:px-2 py-0.5 rounded shadow-2xs border border-slate-100">
                  {doc.type === 'image' ? 'POP-OUT' : 'PREVIEW'}
                </span>
              </div>

              {/* File Info Strip */}
              <div className="bg-[#f4f5f5] p-2 md:p-3 rounded-b-lg border-x border-b border-slate-200/60 flex items-center gap-2 md:gap-3">
                <div className={`relative flex-shrink-0 w-8 h-9 md:w-9 md:h-10 rounded flex flex-col items-center justify-center text-white font-extrabold text-[8px] md:text-[9px] shadow-xs ${doc.type === 'image' ? 'bg-emerald-600' : 'bg-[#d91e36]'}`}>
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-[12px] md:text-[14px] font-semibold text-slate-800 truncate" title={doc.title}>
                    {doc.title}
                  </h3>
                  <p className="text-[10px] md:text-[11px] text-slate-500 mt-0.5 font-medium truncate">
                    {doc.info}
                  </p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 border-t border-slate-100 bg-white divide-x divide-slate-100 text-center font-medium text-[12px] md:text-[14px]">
              <a 
                href={doc.fileUrl} 
                target="_blank" 
                rel="noopener noreferrer" 
                onClick={(e) => handleOpenAction(e, doc)}
                className="py-2.5 md:py-3 text-[#007f5f] hover:bg-slate-50 transition-colors active:bg-slate-100"
              >
                Open
              </a>
              <a 
      ...
      {previewImage && (
        ...
        >
          ...
          <div className="absolute top-0 left-0 right-0 p-4 bg-gradient-to-b from-black/60 to-transparent flex items-center justify-between z-50">
            <div className="text-white font-sans max-w-[60%]">
            ...
            <div className="flex items-center gap-3">
              <a 
                ...
                className="text-white hover:text-emerald-400 font-sans text-xs bg-white/10 px-2.5 py-1.5 rounded-md transition"
              >
                Download ↓
              </a>
              <button 
